    ...
    , ui(new Ui::MainWindow)
{
    ...
    try{
        ...
        try{
            ...
            if (ui->lineEdit_7->text() == "Remote"){
                ...
                ui->lineEdit_9->setText("Remote");
                ui->textEdit->insertPlainText("系統啟動(Remote啟動執行)\n");
            }
        }
        catch(...){}
    }
    ...
    //選PCS
    std::string pcs_name = "delta";//pe
//    std::string pcs_name = "pe";//delta
    //跳頁面(設定)
    ems_win = new EMSLink();
    if (pcs_name == "delta"){
        pcs_win = new PCSLink();
    }else if (pcs_name == "pe"){
//        pcs_win = new PCSLink_pe();
    }
...
//        connect(pcs_win, &PCSLink_pe::send_data, this, &MainWindow::PCSreshow);
//        connect(pcs_win, &PCSLink_pe::send_control, this, &MainWindow::PCScontrol);
    }
...
//        connect(this, &MainWindow::PCSdataupdate_signals, pcs_win, &PCSLink_pe::PCSdataupdate_slots);
    }
    setLED(ui->label_73, 4, 16);
    setLED(ui->label_74, 4, 16);
    setLED(ui->label_75, 4, 16);
    setLED(ui->label_76, 4, 16);
}
...
void MainWindow::PCScontrol(double pcs_controlb, double pcs_resetb, double pcs_control_time_localb)
{
    pcs_controlc = pcs_controlb;
    pcs_resetc = pcs_resetb;
    pcs_control_time_localc = pcs_control_time_localb;
//    qDebug()<<pcs_controlc;
//    qDebug()<<pcs_resetc;
//    qDebug()<<pcs_control_time_localc;
}
...
void MainWindow::MBMSdataupdate_loop()
{
    ...
    setLED(ui->label_73, LEDstate[0], 16);
    setLED(ui->label_74, LEDstate[1], 16);
    setLED(ui->label_75, LEDstate[2], 16);
    setLED(ui->label_76, LEDstate[3], 16);
}
...
void AFCBackground::run(){
    ...
    double soc_now =0;
    int error_code=0;
    qDebug() << name << "runinggg";
    pcs_controlc = 0;
    pcs_resetc = 0;
    pcs_control_time_localc = 0;
    std::thread t1(&dllgc::AFC_run,thread_AFC,
                   std::ref(stopflag),std::ref(error_code),std::ref(ability_test),
                   std::ref(pcs_controlc),std::ref(pcs_resetc),std::ref(pcs_control_time_localc),
                   std::ref(emsdata),std::ref(mbmsdata),std::ref(HMdata),std::ref(pcsdata),std::ref(LEDstate),
                   MBMS_eid,HM_eid,PCS_eid,set_source,
                   std::ref(mode),scale,soc_max,soc_min,
                   System_p_max,System_p_min,System_q_max,System_q_min,
                   f_arr,p_arr,v_arr,q_arr,std::ref(PQ_p_ref),std::ref(PQ_q_ref),
...
void MainWindow::on_pushButton_5_pressed()
{
    ...
    if (running_button_flag==false and running_dont_start_flag==true){
        ...
        AFC_class-> PQ_p_ref = ui->lineEdit_66->text().toDouble();
        AFC_class-> PQ_q_ref = ui->lineEdit_67->text().toDouble();
...
//        qDebug()  << "runing";
    ...
    if (ui->lineEdit_7->text() == "Remote"){
        ui->lineEdit_9->setText("Remote");
    }else if (ui->lineEdit_7->text() == "Local"){
...
{
    ui->lineEdit_66->setText(ui->lineEdit_10->text());
    ui->lineEdit_67->setText(ui->lineEdit_11->text());
    AFC_class-> PQ_p_ref = ui->lineEdit_66->text().toDouble();
    AFC_class-> PQ_q_ref = ui->lineEdit_67->text().toDouble();
}
